...
_ALWAYS_INLINE_ result_t open_file(File **file, str_t file_path, str_t modes,
                                   int flags) {
...
_ALWAYS_INLINE_ result_t file_seek(File *file, qptr_t res, sqword_t off,
                                   size_t whence) {
...
_ALWAYS_INLINE_ result_t file_read(File *file, qptr_t res, bptr_t buf,
                                   size_t num_of_bytes) {
  return merry_file_read(file, res, buf, num_of_bytes);
}

_ALWAYS_INLINE_ result_t file_write(File *file, qptr_t res, bptr_t buf,
                                    size_t num_of_bytes) {

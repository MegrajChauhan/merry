...
_ALWAYS_INLINE_ result_t open_pipe(Pipe **pipe) { return merry_open_merrypipe(pipe);}
...
_ALWAYS_INLINE_ result_t pipe_reopen(Pipe *pipe) { return merry_merrypipe_reopen(pipe);}

_ALWAYS_INLINE_ result_t destroy_pipe(Pipe *pipe) { return merry_destroy_pipe(pipe);}

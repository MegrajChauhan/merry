...
_MERRY_INTERNAL_ MerryUSetBucket *merry_find_bucket(MerryUSet *set, mptr_t key,
...
mresult_t merry_uset_create(MerryUSet **uset, msize_t bucket_count,
  ...
  set->buckets =
      (MerryUSetBucket **)calloc(bucket_count, sizeof(MerryUSetBucket *));
  if (!set->buckets) {
    free(set);
    return MRES_SYS_FAILURE;
  }
...
mresult_t merry_uset_insert(MerryUSet *set, mptr_t key) {
  ...
  MerryUSetBucket *new_bucket =
      (MerryUSetBucket *)malloc(sizeof(MerryUSetBucket));
  if (!new_bucket)
    return MRES_SYS_FAILURE;
  new_bucket->key = key;
...
void merry_uset_destroy(MerryUSet *set) {
  ...
  for (msize_t i = 0; i < set->bucket_count; i++) {
    MerryUSetBucket *bucket = set->buckets[i];
    while (bucket) {

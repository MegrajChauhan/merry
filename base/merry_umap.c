...
mresult_t merry_umap_create(MerryUMap **umap, msize_t bucket_count,
  ...
  map->buckets =
      (MerryUMapBucket **)calloc(bucket_count, sizeof(MerryUMapBucket *));
  if (!map->buckets) {
    free(map);
    return MRES_SYS_FAILURE;
  }
...
void merry_umap_destroy(MerryUMap *map) {
  ...
  for (msize_t i = 0; i < map->bucket_count; i++) {
    MerryUMapBucket *bucket = map->buckets[i];
    while (bucket) {

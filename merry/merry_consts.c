...
mresult_t merry_parse_cmd_option_Graves(mstr_t opt, const mstr_t nxt,
  ...
  case 'l': {
    if (strcmp(opt, "-Glgrpclim") == 0) {
      // nxt must be a value
      if (!nxt) {
        MERR("Expected a value after option %s", opt);
        return MRES_FAILURE;
      }
      msize_t val = strtoull(nxt, NULL, 10);
      if (val == 0) {
        MERR("Invalid value %s provided for option %s", nxt, opt);
        return MRES_FAILURE;
      }
      *used_nxt = mtrue;
      ...
      *used_nxt = mtrue;
      ...
      MERR("Unknown value option %s", opt);
      return MRES_FAILURE;
    }
    break;
  }

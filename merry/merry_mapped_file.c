...
mresult_t merry_mapped_file_map(MerryMappedFile *fmap, mstr_t path, msize_t flags) {
	...
	mresult_t res;
	if ((res = merry_open_file(&fmap->mem_mapped_file.file, path, __FILE_MODE_READ_WRITE, 0)) != MRES_SUCCESS) 
		return res;
	...
	if ((fmem->mem_mapped_file.map = mmap(NULL, len, prot, f, fmap->mem_mapped_file.file->file.fd, 0)) == NULL) 
		return MRES_SYS_FAILURE;
	fmem->mem_mapped_file.flags.update_backing_file = f;
#endif
...
mresult_t merry_mapped_file_unmap(MerryMappedFile *fmap) {
	...
	if (!fmap->mem_mapped_file.map)
		return MRES_NOT_ALLOWED;
#ifdef _USE_LINUX_
	munmap(fmem->mem_mapped_file.map, fmem->mem_mapped_file.len);
#endif
	merry_close_file(fmap->mem_mapped_file.file);
	merry_destroy_file(fmap->mem_mapped_file.file);
	fmem->mem_mapped_file.map = NULL;
	return MRES_SUCCESS;
}

...
Context* context_create(mstr_t file_path) {
	...
	MerryFile *file;
	if (merry_open_file(&file, file_path, _MERRY_FOPEN_READ_, 0) != MRES_SUCCESS) {
	...
	if (!ctx->file_stream) {
		MERR("[%s]: Failed to allocate buffer for file(length=%zu)", file_path, len_of_file);
		free(ctx);
		merry_destroy_file(file);
		return NULL;
	}
	if (merry_file_read(file, NULL, ctx->file_stream, len_of_file) != MRES_SUCCESS) {
...
mbool_t context_process_file(Context *ctx) {
	...
	while (curr.type != TOK_ERR && curr.type != TOK_EOF) {
		printf("TOK_TYPE: %zu\nTOK_LINE: %zu\nTOK_COL: %zu\nTOK_VAL: ", curr.type, curr.lnum, curr.col_st);

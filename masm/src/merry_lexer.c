...
_MERRY_INTERNAL_ void lexer_handle_decimal(Lexer *l, Token *t) {
    size_t dot_count = 0;
    while (l->curr < l->len && (misnum(l->stream[l->curr]) || l->stream[l->curr] == '.')) {
        if (l->stream[l->curr] == '.')
            dot_count++;
        if (dot_count > 1) {
            MERR("[LINE:%zu]: Floating point number cannot have multiple floating points", l->lnum);
            t->type = TOK_ERR;
            return;
        }
        lexer_update(l);
...
_MERRY_INTERNAL_ void lexer_handle_binary(Lexer *l, Token *t) {
    ...
    msize_t len = 0;
    while (l->curr < l->len && (l->stream[l->curr] == '0' || l->stream[l->curr] == '1')) {
        lexer_update(l);
        len++;
    }
    if (len == 0) {
    	MERR("[LINE:%zu]: Expected an actual bit sequence after prefix '0b'", l->lnum);
    	t->type = TOK_ERR;
    	return;
    }
...
_MERRY_INTERNAL_ void lexer_handle_hex(Lexer *l, Token *t) {
    ...
    lexer_update(l);
    msize_t len = 0;
    while (l->curr < l->len && (misnum(l->stream[l->curr]) || ((l->stream[l->curr] >= 'A' && l->stream[l->curr] <= 'F') || (l->stream[l->curr] >= 'a' && l->stream[l->curr] <= 'f')))) {
    ...
    if (len == 0) {
        MERR("[LINE:%zu]: Expected an actual hex sequence after prefix '0x'", l->lnum);
        t->type = TOK_ERR;
        return;
    }
    t->type = TOK_NUM_HEX;
...
_MERRY_INTERNAL_ void lexer_handle_octal(Lexer *l, Token *t) {
    ...
    lexer_update(l);
    msize_t len = 0;
    while (l->curr < l->len && (misnum(l->stream[l->curr]) && ((l->stream[l->curr] >= '0' && l->stream[l->curr] <= '7')))) {
        lexer_update(l);
        len++;
    }
    if (len == 0) {
        MERR("[LINE:%zu]: Expected an actual octal sequence after prefix '0o'", l->lnum);
        t->type = TOK_ERR;
        return;
    }
...
Token lexer_next(Lexer *l) {
    ...
    if (l->stream[l->curr] == '+') {
      ...
      lexer_update(l);
        ...
        else if (l->stream[l->curr] == '0' && l->len > peek && l->stream[peek] == 'b')
            lexer_handle_binary(l, &res);
        else if (l->stream[l->curr] == '0' && l->len > peek && l->stream[peek] == 'o')
            lexer_handle_octal(l, &res);
        else if (l->stream[l->curr] == '0' && l->len > l->curr && misalpha(l->stream[peek])) {
    	...
    	MERR("[LINE:%zu]: Cannot build a token from this '%c'", l->lnum, l->stream[l->curr]);
    	res.type = TOK_ERR;
    }
